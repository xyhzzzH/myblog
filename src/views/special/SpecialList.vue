            ...
            <template #op="{ row }">
              <div class="op">
                <a href="javascript:void(0)" class="a-link" @click="showEdit('update', row)"
                  v-if="userInfo.userId == row.userId || userInfo.roleType == 1">修改</a>
                <span v-else>--</span>
                <el-divider direction="vertical" />
                <a href="javascript:void(0)" class="a-link" @click="del(row)"
                  v-if="userInfo.userId == row.userId || userInfo.roleType == 1">删除</a>
                <span v-else>--</span>
              </div>
            </template>
          </Table>
      ...
      <el-col :span="10">
        <el-card class="box-card">
          ...
          <div :style="{ 'margin-bottom': '5px' }">
            <el-alert title="红色代表草稿绿色代表已发布、拖动文章修改排序" type="info" show-icon :closable="false" />
          </div>
          <el-tree class="tree-panel" ref="refTree" :data="blogList" defaultExpandAll node-key="blogId"
            :expand-on-click-node="false" :props="treeProps" :highlight-current="true" draggable @node-drop="blogDrag">
            <template #default="{ data }">
              <span class="custom-node-style" style="width: 100%;">
                ...
                <span class="node-op">
                  ...
                  <template v-else>
                    ...
                    <a class="a-link" href="javascript:void(0)" @click="editBlog('edit', data)"
                      v-if="userInfo.userId == data.userId || userInfo.roleType == 1">修改</a>
                    <span v-else>--</span>
                    <el-divider direction="vertical" />
                    <a class="a-link" href="javascript:void(0)" @click="delBlog(data)"
                      v-if="userInfo.userId == data.userId || userInfo.roleType == 1">删除</a>
                    <span v-else>--</span>
                    <el-divider direction="vertical" />
    ...
    <Dialog :show="dialogConfig.show" :title="dialogConfig.title" :buttons="dialogConfig.buttons" width="500px"
      ...
      <el-form :model="formData" :rules="rules" ref="formDataRef" label-width="80px">
        ...
        <el-form-item label="简介" prop="categoryDesc">
          <el-input v-model="formData.categoryDesc" type="textarea" placeholder="请输入简介"
